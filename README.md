...
**Q. APKのファイルサイズが非常に大きいのはなぜですか？**
A. 本アプリは、Android OS 内蔵の標準ブラウザエンジン（WebView）ではなく、Mozilla の独立した強力なブラウザエンジンである「GeckoView」をアプリ自体に内包（バンドル）して動作しているためです。このエンジンを採用することで、PC向けの高度なブラウザ拡張機能（LiveChat Flusherなど）をスマホ上で安定して動作させることが可能になっていますが、その代償としてアプリのベースサイズが通常よりも大きくなっています。
